...
/*
 * 사용자들이 GUI 프로그램에서 이벤트를 일으니면 1차적으로는 OS(MAC, WINDOW, LINUX)가 감지하여
 * 해당 프로그램에 전달해 준다.
 * 우리의 경우 JVM이 이벤트를 정보를 전달받는데, 이때 JVA은 자바에서 사용되는 이벤트로 재해석하여
 * 정보를 담고 있는 이벤트 객체의 인스턴스를 생성한다. 예를 들면 클릭이벤트 정보를 OS로 부터 전달받으면
 ...
 * 이 리스너를 사용하기위해서는 인터페이스로 되어 잇으므로, 자식 객체를 재정의하여 자식을 new 해야 한다.
 * */

//인터페이스는 몸체없는 메서드만을 보유하므로, 상당히 추상적이다. 따라서 추상메서드를 우리가 재정의 하여
//인스턴스를 생성해보자
public class MyActionListener implements ActionListener {

	//이 메서드는 사용자에 의해 액션이벤트가 발생할때마다, 호출되어 진다.
	//이때, jvm 이 생성한 이벤트 정보 객체인 ActionEvent 인스턴스가 아래의 매개변수인 e 변수로
	//대입이 되어진다. 따라서 개발자는 더많은 이벤트 전보를 보고싶다면 매개변수인 e 를 이용하면 된다
