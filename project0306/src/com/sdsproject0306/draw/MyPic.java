package com.sdsproject0306.draw;
...
public class MyPic extends JFrame implements ActionListener {
	...
	private void drawLine() {
		...
		//Canvas의 값을 위완 같이 변경 한 후, 사용자에 의한 paint() 메서드 호출이 아닌
		//(윈도우창 등의 그림의 변화를 유발하는 행동), 프로그래밍적으로 paint() 메서드를 호출해보겠다.
		
		//그림 그리는 행위는 시스템에 의해 컴포넌트가 알아서 하고 또 갱신하는 행위 조차도 시스템
		//에 의해서 처리되는 것이므로, 개발자는 절대로 paint()메서드를 호출해선 안되고 호출하는 법도 없다.
