...
public class Painter extends JFrame implements MouseMotionListener{
	...
	/*
	 ...
	 자바에서는 collection framework 에서 해결 가능
	  ■ collection framework 란? 자바의 자료구조 패키지임
	     - 객체를 모아서 처리할때 유용한 기능들을 지원하는 객체 java.util
	     - 이 세상의 모든~~ 객체가 모여있는 모습을 단 3가지 관점으로 바라본다.
	     1) 순서 있게 모여 있는 모습 -List 유형
	     2) 순서 없이 모여 있는 모슴 - Set 유형
	     ...
	     차이점이라면 다음과 같다.
	     1) 크기가 동적으로 늘어남
	     2) 배열은 기본 자료형도 담을 수 있지만, ArraryList은 오직 객체만 담는다.
	...
	public void mouseDragged(MouseEvent e) {
		...
		int[] pos = {can.x, can.y}; //크기가 2인 정수 배열, 점1개 표현
