...
public class MyTableModel extends AbstractTableModel{
	/*
	데이터가 되는 이차원배열
	자바에서 배열은 길이가 고정되므로, 데이터가 추후 지속적인 변경사항이 발생한다면 배열보다는
	보다 유연한 컬렉션 프레임웍을 이용해본다
	-컬렉션 프레임웍? JAVA에서 객체만을 모아서 처리할때 유용한 기능을 지원하는 패키지
	이 패키지 안에는 여러가지 객체들이 지원되는데, 유형은 3가지로 나뉜다
	1) List 형 - 순서있는 집합을 표현한 객체 , List 자체는 인터페이스이므로, 이를 상속받는 하위 자식을 이용
					배열과 거의 같지만, 길이가 유동적, 오직 객체만을 담을 수 있다.
	2) Set 형 - 순서없는 집합을 표현한 객체 , Set 자체는 인터페이스이므로, 이를 상속받는 하위 자식을 이용
	3) Map 형 - key-Value의 쌍으로 집합을 표현한 객체 , Map 자체는 인터페이스이므로, 이를 상속받는 하위 자식을 이용
	*/
	
	//배열을 대체할 보다 유연한 컬렉션 프레임웍의 List의 자식 중 하나인 ArratyList를 사용해본다.
	...
	public Object getValueAt(int row, int col) {
		...
		String[] member = (String[])obj; //Object형태로 들어가 있던 일차원배열을 다시 형변환 해서 복원
		return member[col]; //이차원 배열을 이루고 있는 각 칸을 JTable이 이메서드의 반환값을 호출한다.
	}
