...
public class MemberRegist extends JFrame{
	...
	public MemberRegist() {
		...
		//JTable의 3번째 생성자 방법인 즉 생상자에 이차원배열을 대입하는 방식은,
		//Table이 보여줄 데이터를 고정하기 때문에, 데이터를 변경한다거나 교체가할 경우 불가능함
		//유지보수성 떨어짐, 예를 들면 과일을 보여주고 싶다면 과일용 JTable 이차원배열을 또 만들어야 한다.
		//따라서 4번째 생성자를 이용해본다. TableModel 을 이용하면 디자인인(View) JTable과
		//그 안에 보여질 데이터(Model)을 분리시켜 개발할 수 있으므로, 추후 다른 종류의 데이터를 보여
		//주어야 한다 해도, 코드가 크게 바뀔것이 없다. 특히 기존의 JTable 관련 코드 유지가능
		...
		add(scroll); //센터에 스크롤 부착(테이블 포함)
		
		//버튼과 리스너 연결
		bt.addActionListener(new ActionListener() {

			public void actionPerformed(ActionEvent e) {
				//System.out.println("나 눌렀냐");
				regist();
			}
		});
	...
	//회원 추가하기
	public void regist() {
		/*배열은 처음에 생성 시 그 크기를 정해놓으면, 수정이 불가능하다
		...
		System.out.println("현재 누전된 사원수"+model.list.size());
