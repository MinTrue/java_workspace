...
/*
 * 자바의 컴포넌트 중, Panel 은 판자이며 다른 컴포넌트(버튼, 체크박스..)과 다르게
 * 배치 능력을 보유하고 있다. 즉 배치관리자(BorderLayout, GridLayout, FlowLayout)의 적용 가능
...
public class PanelTest {
	...
	public static void main(String[] args) {
		Frame frame = new Frame("Panel Study"); //윈도우창 생성과 이름 부여하기
		
		frame.setLayout(new FlowLayout()); //디폴트가 BorderLayout이므로, 패널 테스트에 저장.
		
		//div 역할을 수행할 Panel 을 두개 준비하자
		Panel p1 =new Panel(); //첫번째 판자
		Panel p2 =new Panel(); //두번째 판자
		/*판자의 능력
		 * 1) 배치관리자를 사용할 수 있는 능력
		 * 		만일 개발자가 아무것도 배치관리자를 적용하지 않으면 패널을 디폴트로 FlowLayout 적용
		 * 2) Frame의 능력인줄만 알았던 컴포넌트 add() 능력도 가지고 있다.
		 *     즉 Container 가 될 수 있다. (parent가 될 수 있다. 주의-Super용어 랑 구분하자)
		 * */
		...
		frame.add(p1); //윈도우에 판자 1부착
		frame.add(p2); //윈도우에 판자 2부착
