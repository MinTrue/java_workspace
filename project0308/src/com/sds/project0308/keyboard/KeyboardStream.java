 ...
 * 이유? 하드웨어는 os가 먼저 처리하고, 자바는 단지 os에서 만들어놓은 스트림을 연걸하는 것 뿐이다.
 * */
public class KeyboardStream {

	public static void main(String[] args) {
		//키보드와 입력스트림 연결하기
		//키보드는 별도의 클래스로 지원될 대상이 아니다 따라서 입력스트림 중 가장 최상위의 추상 클래스인
		//그냥 입력스트림으로 받자
		InputStream is=System.in; //이미 OS 차원에서 생성해 놓은 입력스트림을 얻어오자
		//OS 차원에서 얻어놓은 입력 스트림은 표준을 따르기 때문에 자바 언어가 별도의 하드웨어를 인식
		//할 필요도 없이 그냥 모든 디바이스에 대한 스트림은 InPutStream으로 연결만 하면 된다.
		
		//문자 기반의 입력스트림
		InputStreamReader reader =new InputStreamReader(is);// 2단계 빨대(문자본다)
